...
설치본은 `skills/ste-ko/SKILL.md` 한 장입니다. 표면 네 개, 항상 지키는 열 가지, 버리는 것과 지우지 않는 것, 규격을 자동으로 푸는 조건, 표면별 예문이 들어 있습니다. 규칙 70개 전체는 SKILL.md가 아니라 `SPEC.md`에 둡니다. 매 턴 입력에 실리는 분량을 줄이기 위해서입니다.
